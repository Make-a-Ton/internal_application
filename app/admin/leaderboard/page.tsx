    ...
    return (
        <div className="min-h-screen p-8">
            <div className="mb-8">
                ...
                <p className="text-gray-500 mt-1">Overall team rankings from all judges</p>
            </div>
            ...
            {leaderboard.length === 0 ? (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
                    ...
                    <p className="text-sm text-gray-400 mt-1">Scores will appear here once judges start scoring.</p>
                </div>
            ) : (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="grid gap-4 px-6 py-3 bg-gray-50 border-b border-gray-100 text-xs font-bold text-gray-500 uppercase tracking-wider"
                    ...
                    {leaderboard.map((entry, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`grid gap-4 px-6 py-4 border-b border-gray-50 items-center hover:bg-gray-50 transition-colors ${i < 3 ? "bg-[#5C0124]/[0.02]" : ""}`}
                                style={{ gridTemplateColumns: `60px 1fr repeat(${judges.length}, 100px) 100px 80px` }}
                            >
                                <span className="text-2xl">{medal}</span>
                                <span className="font-bold text-gray-900">{entry.teamName}</span>
                                {judges.map(j => (
                                    <span key={j.id} className="text-sm font-medium text-gray-600">
                                ...
                                <span className="text-sm text-gray-500">{entry.judgeCount}/{judges.length}</span>
                            </motion.div>
                        );
                    })}
                </div>
