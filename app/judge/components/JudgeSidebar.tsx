...
import { LayoutDashboard, ClipboardCheck, Trophy, LogOut } from "lucide-react";
...
export default function JudgeSidebar() {
    ...
    useEffect(() => {
        ...
        if (id === "judge-1") setJudgeName("Judge 1");
        else if (id === "judge-2") setJudgeName("Judge 2");
        else if (id === "judge-3") setJudgeName("Judge 3");
    }, []);

    const handleLogout = () => {
        localStorage.removeItem("makeaton_judge_id");
        router.push("/");
    };
    ...
    return (
        <aside className="fixed left-0 top-0 bottom-0 w-64 bg-[#5C0124] text-[#F4E4BC] flex flex-col z-40">
            ...
            <div className="p-6 border-b border-[#7A2840]">
                ...
                <p className="text-xs text-[#D4AF37] uppercase tracking-widest mt-1">Judge Panel — {judgeName}</p>
            </div>
